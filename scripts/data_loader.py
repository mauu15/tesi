...
def read_operators(file_path):
    ...
    with open(file_path, mode='r', newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            op = {
                "id": row["id"],
                "max_weekly_hours": int(float(row["max_weekly_hours"])) if row["max_weekly_hours"] else 0,
                "available_days": row["available_days"],  # eventuale conversione a lista se necessario
    ...
    with open(file_path, mode='r', newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            req = {
                ...
                "n_operators_required": int(row["n_operators_required"]) if row["n_operators_required"] else 1,
