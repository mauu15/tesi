...
def grs_variants(operators, requests, patients, shift_end, down_time_true, tau, k):
    ...
    :return: routing_cost e overtime_cost.
    """
    ...
    return total_routing_cost, total_overtime_cost, sum(op["do_k"][k] for op in operators)
...
def compute_f_oi(operator, request, waiting_time, theta=0.37, tau=None, down_time_true=False):
    ...
    if operator["wo"] + service_time + travel_time + waiting_time > operator["Ho"]:
        ...
        overtime_penalty = op_cost_per_minute * (service_time + travel_time + waiting_time+min(operator["wo"] - operator["Ho"], 0))
    else:
        overtime_penalty = 0
    ...
    overtime_cost = overtime_penalty
